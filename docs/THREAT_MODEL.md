...
**Mitigation:** Server-side validation automatically rejects fields containing phone numbers, GPS coordinate patterns, URLs, email addresses, street addresses, vehicle plates, and violent language. The record is never stored on rejection.
...
**Mitigation:** Automatic policy checks (quality grading, rate limiting at 10/IP/hour, honeypot field) reduce automated spam. Quality D records are rejected outright and never stored. The registry accepts only structured metadata with no free-text fields.
...
**Mitigation:** Records can only be retracted by the holder using the retraction token generated at proof package creation. The server stores only the SHA-256 of the token; the raw token is never transmitted after creation. Retraction uses `crypto.timingSafeEqual` for token comparison and is rate-limited to 5 attempts per record per 15 minutes.
...
**Risk:** If the server or database is fully compromised, an attacker could access all public record metadata.

**Mitigation in design:** Original files are never stored. The worst-case leak is public metadata (event type, evidence type, country) that was already intentionally submitted for public disclosure.

---

### 2. Network-level interception of the submission
...
### 3. Unsafe metadata submitted by the user
...
**Mitigation in design:** The tool warns users to use general context only and provides "withheld" options. Server-side validation catches common unsafe patterns (phone numbers, GPS, addresses, plates), but cannot catch all user-supplied context.

---

### 4. Device seizure before download
...
### 5. Browser or device compromise
...
### 6. Compression-broken verification
...
1. Set `PUBLIC_APP_ORIGIN` to your production domain.
2. Rotate `SESSION_SECRET` periodically.
3. Monitor deployment logs for unexpected submission volumes or error spikes.
4. Consider external security review before deploying for high-risk use cases.
...
- All public record metadata (event type, evidence type, country, etc.)
